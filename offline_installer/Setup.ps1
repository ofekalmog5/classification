...
function Start-Install([string]$dir, [bool]$skipAI, [bool]$forceCuda, [bool]$installGpu) {
    ...
    SetStatus "Step 1 / 9 — Extracting Python 3.11.9..."; SetPct 2
    Log ""; Log "=== [1/9] Extracting Python ===" "White"

    $zipPath = Join-Path $PREREQ "python-3.11.9-embed-amd64.zip"
    if (-not (Test-Path $zipPath)) {
        Log "ERROR: $zipPath not found" "Red"; return $false
    }
    ...
    Log "Python extracted to: $pyDir" "Green"

    # ── 2. Enable site-packages ───────────────────────────────
    SetStatus "Step 2 / 9 — Configuring Python..."; SetPct 7
    Log ""; Log "=== [2/9] Enabling site-packages ===" "White"
    ...
    if ($pth) {
        ...
        Log "Patched $($pth.Name)" "Green"
    } else {
        Log "WARNING: ._pth file not found — pip may not work" "Yellow"
    }

    # ── 3. Bootstrap pip ─────────────────────────────────────
    SetStatus "Step 3 / 9 — Installing pip..."; SetPct 12
    Log ""; Log "=== [3/9] Bootstrapping pip ===" "White"

    $getPip = Join-Path $PREREQ "get-pip.py"
    if (-not (Test-Path $getPip)) {
        Log "ERROR: get-pip.py not found at $getPip" "Red"; return $false
    }
    $rc = Invoke-Step $pyExe "`"$getPip`" --no-warn-script-location" "python get-pip.py"
    if ($rc -ne 0) { Log "WARNING: get-pip.py returned $rc (may be OK)" "Yellow" }

    # Upgrade pip/setuptools/wheel from offline cache
    Invoke-Step $pyExe `
        "-m pip install --no-index --find-links `"$PKG`" --upgrade pip setuptools wheel" `
        "upgrading pip/setuptools/wheel" | Out-Null
    Log "pip ready." "Green"
